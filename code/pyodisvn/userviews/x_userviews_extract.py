...
Actually this above approach has been hacked - basically I do not have permissions so its a bit moot.
Instead I output to c:\temp and ask nickto run them.
...
>>> connfinger = tdlib.getConn(dsn=MOI_user_passwords.DSNSTRINGS['FINGERPRINT'])
...
'%s_E_CODE',
    ...
    for context in td_contexts:
        for dbtype in td_dbtypes:
            if dbtype.find("USER_VIEWS") == -1 : continue
    ...
    fo = open(r'C:\$WELLKNOWNINSURER_DATA\UserViews\nicktorun.sql', 'w')
    for dbname, conntype, dbasetype in dbases_to_parse:
        
        print
        print "Extracting for ", dbasetype, dbname, conntype
        conn = tdlib.getConn(MOI_user_passwords.DSNSTRINGS[conntype])
        tables = get_tables(conn, dbname)
        

        for dbname, tablename, tabletype in tables:
            ##this bit putputs show mick can run it - I do not have permissions
 
            tablename = tablename.strip()
            dbname = dbname.strip()
            fo.write("SHOW VIEW %s.%s;\n" % (dbname, tablename))
