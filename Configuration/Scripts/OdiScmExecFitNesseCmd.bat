...
set TEMPFILE=%TEMPDIR%\fitnesse_jars.txt

dir /b "%ODI_SCM_TOOLS_FITNESSE_HOME%\lib\fitnesse-*.jar" > "%TEMPFILE%"
if ERRORLEVEL 1 (
	echo %EM% cannot find Fitnesse JAR file in Fitnesse lib directory ^<%ODI_SCM_TOOLS_FITNESSE_HOME%\lib^> 1>&2
	goto ExitFail
)

set /p ODI_SCM_FITNESSE_JAR_FILE=<"%TEMPFILE%"

set FITNESSECMD="%ODI_SCM_TOOLS_FITNESSE_JAVA_HOME%\bin\java.exe" -jar "%ODI_SCM_TOOLS_FITNESSE_HOME%\lib\%ODI_SCM_FITNESSE_JAR_FILE%"
